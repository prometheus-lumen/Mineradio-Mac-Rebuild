...
function applyRendererPowerMode(): void {
...
function getRenderPixelLoad(): number {
  var ratio = getRenderPixelRatio();
  return Math.max(1, innerWidth * innerHeight) * ratio * ratio;
}

function markRenderInteraction(reason = 'interaction', holdMs?: number): void {
...
function isRenderInteractionActive(now = performance.now()): boolean {
  return (now || performance.now()) < renderInteractionBoostUntil;
}

function applyControlGlassChromaticOffset(): void {
  if (!fx) return;
...
function generateControlGlassDisplacementMap(width: number, height: number, radius: number): string {
...
function updateGlassDisplacementMapForElement(el: Element | null, img: Element | null, stateKey: string): void {
  if (!el || !img) return;
...
function updateControlGlassDisplacementMap(): void {
...
function initControlGlassSurface(): void {
