function triggerPresetParticleTransition(fromPreset: number, toPreset: number): void {
  ...
  presetTransition.duration = toPreset === 5 ? 0.30 : (toPreset === HEXAGRAM_PRESET_INDEX || toPreset === PARTICLE_CLOCK_PRESET_INDEX || toPreset === TOPOGRAPHY_PRESET_INDEX ? 0.42 : 0.24);
  presetTransition.from = fromPreset;
  presetTransition.to = toPreset;
  var newVisual = toPreset >= 4;
  var wallpaperFlow = toPreset === 5 || toPreset === HEXAGRAM_PRESET_INDEX || toPreset === PARTICLE_CLOCK_PRESET_INDEX || toPreset === TOPOGRAPHY_PRESET_INDEX;
...
function tickPresetTransition(): void {
  ...
  var wallpaperFlow = presetTransition.to === 5 || presetTransition.to === HEXAGRAM_PRESET_INDEX || presetTransition.to === PARTICLE_CLOCK_PRESET_INDEX || presetTransition.to === TOPOGRAPHY_PRESET_INDEX;
...
function setPreset(p: number, opts: PresetOptions = {}): void {
  ...
  if (changed && !opts.preserveCamera) {
    ...
    else if (p === TOPOGRAPHY_PRESET_INDEX) { orbit.userRadius = 10.35; orbit.userPhi = 0.252; orbit.userTheta = -0.387; orbit.baselineRadius = 10.35; orbit.baselinePhi = 0.252; }
    else              { orbit.userRadius = 6.6; orbit.userPhi = 0.08; orbit.userTheta = 0.0; orbit.baselineRadius = 6.6; orbit.baselinePhi = 0.08; }
    orbit.baselineTheta = p === 5 ? -0.52 : (p === 6 ? 0.18 : (p === TOPOGRAPHY_PRESET_INDEX ? -0.387 : 0.0));
  }
