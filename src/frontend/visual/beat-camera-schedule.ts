function scheduleBeatCamera(beat: BeatEventLike | CameraBeatEvent, source: string): void {
  if (!fx.cinema) return;
  ...
  var livePreview = Boolean(isLiveSource && typeof beat !== 'number' && beat.preview);
  var dj = djMode.active && (isLiveSource || isDjMapSource || typeof beat !== 'number' && beat.dj);
  ...
  if (isDjMapSource) {
    ...
    var beatStep = typeof beat !== 'number' ? beat.step : 0;
    var djMinGap = Math.max(0.255, Math.min(0.470, beatStep ? beatStep * 0.52 : 0.320));
