var SPLASH_VERTEX_SHADER_SOURCE = `
...
var SPLASH_FRAGMENT_SHADER_SOURCE = `
...
function compileSplashShader(gl: WebGLRenderingContext, type: number, source: string): WebGLShader | null {
...
function initMineradioSplashWebgl(canvas: HTMLCanvasElement): boolean {
  ...
  try {
    ...
    if (!context) context = canvas.getContext('experimental-webgl') as WebGLRenderingContext | null;
  } catch (error) {}
  if (!context) return false;
  var vertex = compileSplashShader(context, context.VERTEX_SHADER, SPLASH_VERTEX_SHADER_SOURCE);
  var fragment = compileSplashShader(context, context.FRAGMENT_SHADER, SPLASH_FRAGMENT_SHADER_SOURCE);
  ...
  if (!context.getProgramParameter(program, context.LINK_STATUS)) {
    console.warn('Splash shader link failed:', context.getProgramInfoLog(program));
...
function drawMineradioSplashWebgl(elapsed: number): void {
  var gl = splashGl;
  var program = splashGlProgram;
  var buffer = splashGlBuffer;
  var uniforms = splashGlUniforms;
  var canvas = splashCanvas;
  if (!gl || !program || !buffer || !uniforms || !canvas) return;
