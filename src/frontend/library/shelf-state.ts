...
function shelfCueCenter(): Point2D {
  var r = shelfCueRect();
  return { x: r.left + r.width * 0.58, y: r.top + r.height * 0.50 };
}

function setShelfPinnedOpen(open: boolean, immediate = false): void {
  var nextOpen = !!open;
...
function clearShelfPreviewOnPointerExit(): void {
...
function suppressShelfPreviewForPlaybackSwitch(): void {
  if (!shelfManager || !shelfManager.getMode || shelfManager.getMode() !== 'side') return;
...
function safeShelfRebuild(reason: string, asyncCards = true): boolean {
...
function scheduleShelfRebuild(reason: string, asyncCards = true): void {
...
function safeShelfCloseContent(reason: string): boolean {
...
function safeSwitchPlaylistTab(tab: string, reason: string): boolean {
  try {
...
function __mineradioInitLibraryShelfState36(): void {
  // ============================================================
  ...
  document.addEventListener('mouseout', function(e: MouseEvent) {
    var legacyEvent = e as MouseEvent & { toElement?: EventTarget | null };
    if (!e.relatedTarget && !legacyEvent.toElement) clearShelfPreviewOnPointerExit();
  });
}
