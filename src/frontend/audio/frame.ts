...
function tickPodcastDjBeatMap(): void {
...
function triggerScheduledBeat(beat: BeatEventLike): void {
  ...
  var djPulse = typeof beat !== 'number' && !!beat.dj;
...
function updateHomeAudioVisual(_dt: number): void {
  ...
  for (var i = 0; i < bars.length; i++) {
    ...
    bar.style.height = Math.max(4, prev * 18) + 'px';
    bar.style.opacity = String(clampRange(0.36 + prev * 0.68, 0.32, 1));
  }
}
