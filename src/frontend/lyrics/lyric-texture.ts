...
function requireCanvas2D(canvas: HTMLCanvasElement): CanvasRenderingContext2D {
  var context = canvas.getContext('2d');
  if (!context) throw new Error('Canvas 2D context is unavailable');
  return context;
}

function isLyricCanvasTexture(texture: THREE.Texture): texture is LyricCanvasTexture {
  return 'userData' in texture;
}

function disposeLyricTexture(texture?: THREE.Texture | null): void {
  if (!texture || (isLyricCanvasTexture(texture) && texture.userData.mineradioShared)) return;
  texture.dispose();
}

function disposeLyricMesh(mesh?: THREE.Object3D | null): void {
  if (!mesh) return;
  if (mesh.parent) mesh.parent.remove(mesh);
  mesh.traverse(function(node: THREE.Object3D): void {
    var obj = node as LyricRenderableObject;
    if (obj.material) {
      if (Array.isArray(obj.material)) {
        obj.material.forEach(function(m: LyricDisposableMaterial): void { if (m.map) disposeLyricTexture(m.map); m.dispose(); });
      } else {
...
function applyStonePrintTexture(ctx: CanvasRenderingContext2D, W: number, H: number, fontSize: number): void {
  ...
  var nctx = requireCanvas2D(noise);
...
function makeLyricMask(text: string): LyricMask {
...
function makeLyricReadabilityTexture(mask: LyricMask): THREE.Texture {
  var canvas = document.createElement('canvas');
  ...
  function strokeLines(dx: number, dy: number): void {
