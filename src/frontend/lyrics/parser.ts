...
interface LyricLine {
  ...
  duration?: number;
  charCount?: number;
  source?: 'lrc' | 'yrc-line' | 'yrc-word' | 'custom-lrc' | 'custom-text';
  words?: LyricWord[];
  fallback?: boolean;
}
...
function lyricTagTimeToSeconds(min: string, sec: string, frac?: string): number {
  ...
  if (frac) time += (parseInt(frac, 10) || 0) / Math.pow(10, Math.min(3, frac.length));
  return time;
}
...
function finalizeLyricLineDurations(lines: LyricLine[]): LyricLine[] {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (!Number.isFinite(line.duration) || Number(line.duration) <= 0) line.duration = inferred;
...
function parseYrcText(text: string): LyricLine[] {
  const lines: LyricLine[] = [];
  String(text || '').split(/\r?\n/).forEach((rawLine) => {
