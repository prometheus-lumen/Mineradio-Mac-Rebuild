...
function renderMusicCacheInfo(info) {
  var directory = document.getElementById('music-cache-directory');
  var summary = document.getElementById('music-cache-summary');
  if (directory) {
    directory.textContent = info && info.directory ? info.directory : '无法读取缓存目录';
    directory.parentElement.title = info && info.directory ? ('打开 ' + info.directory) : '无法读取缓存目录';
  }
  if (summary) {
...
async function refreshMusicCacheInfo() {
...
async function openMusicCacheDirectory() {
  var api = window.desktopWindow;
...
async function confirmAndClearMusicCache() {
  var api = window.desktopWindow;
  ...
  if (!confirmed) return;
  var button = document.getElementById('music-cache-clear-btn');
  if (button) {
